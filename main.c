  ...
  for (cell = 0; cell < size (mesh); cell++)
    if (get_up (mesh, cell) == -1) /// w3.2
    ...
    else /// w1 w4 w5
      {
        ...
        /// v
        coef = V__GetCmp (u, 3 * cell + 1) / tau + (8. * VISC) / (3. * get_h1 (mesh) * get_h1 (mesh)) + (2. * VISC) / (get_h2 (mesh) * get_h2 (mesh));
        Q__SetEntry (A, 3 * cell + 1, 0, 3 * cell + 2, coef);
        /// v_n+1
        coef = (1. /(3. * get_h1 (mesh))) * (0.5 * V__GetCmp (u, 3 * cell + 1) * V__GetCmp (u, 3 * cell + 2) + 0.5 * V__GetCmp (u, 3 * neigh4 + 1) * V__GetCmp (u, 3 * neigh4 + 2) - 4. * VISC / get_h1 (mesh));
        Q__SetEntry (A, 3 * cell + 1, 1, 3 * neigh4 + 2, coef);
        /// v_n-1
        coef = (1. /(3. * get_h1 (mesh))) * (0.5 * V__GetCmp (u, 3 * cell + 1) * V__GetCmp (u, 3 * cell + 2) + 0.5 * V__GetCmp (u, 3 * neigh3 + 1) * V__GetCmp (u, 3 * neigh3 + 2) + 4. * VISC / get_h1 (mesh));
        Q__SetEntry (A, 3 * cell + 1, 2, 3 * neigh3 + 2, -coef);
        /// v_m+1
        coef = (1. / get_h2 (mesh)) * (0.25 * V__GetCmp (u, 3 * cell + 1) * V__GetCmp (u, 3 * cell + 3) + 0.25 * V__GetCmp (u, 3 * neigh1 + 1) * V__GetCmp (u, 3 * neigh1 + 3) - VISC / get_h2 (mesh));
        Q__SetEntry (A, 3 * cell + 1, 3, 3 * neigh1 + 2, coef);
        /// v_m-1
        coef = (1. / get_h2 (mesh)) * (0.25 * V__GetCmp (u, 3 * cell + 1) * V__GetCmp (u, 3 * cell + 3) + 0.25 * V__GetCmp (u, 3 * neigh2 + 1) * V__GetCmp (u, 3 * neigh2 + 3) + VISC / get_h2 (mesh));
        ...
        /// fill vector b
        coef = (V__GetCmp (u, 3 * cell + 1) * V__GetCmp (u, 3 * cell + 2)) / tau;
        coef += (0.25 * V__GetCmp (u, 3 * cell + 2) * V__GetCmp (u, 3 * cell + 3) * (V__GetCmp (u, 3 * neigh2 + 1) - V__GetCmp (u, 3 * neigh1 + 1))) / get_h2 (mesh);
        coef += (0.25 * V__GetCmp (u, 3 * cell + 1) * V__GetCmp (u, 3 * cell + 2) * (V__GetCmp (u, 3 * neigh2 + 3) - V__GetCmp (u, 3 * neigh1 + 3))) / get_h2 (mesh);
        coef += (0.5 * V__GetCmp (u, 3 * cell + 2) * V__GetCmp (u, 3 * cell + 2) * (V__GetCmp (u, 3 * neigh4 + 1) - V__GetCmp (u, 3 * neigh3 + 1))) / (3. * get_h1 (mesh));
        coef += (VISC * (V__GetCmp (u, 3 * neigh24 + 3) - V__GetCmp (u, 3 * neigh23 + 3) - V__GetCmp (u, 3 * neigh14 + 3) + V__GetCmp (u, 3 * neigh13 + 3))) / (4. * get_h1 (mesh) * get_h2 (mesh));
        coef += V__GetCmp (u, 3 * cell + 1) * calc_f1 (cur_t, get_x (mesh, cell), get_y (mesh, cell));
        V__SetCmp (b, 3 * cell + 1, coef);
